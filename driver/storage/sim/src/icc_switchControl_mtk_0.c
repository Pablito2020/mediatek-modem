...
//	SimCard->gpt_handle = usim_dcb->gpt_handle;
	...
	if (SimCard->result == SIM_SUCCESS)
	{
		...
		while(1)
		{
			...
			{
#if	!defined( __MAUI_BASIC__)
            tst_sys_trace("Bad card/Hw contact issue, cause PTS error. Enter recovery process");
#else
            dbg_print("Bad card/Hw contact issue, cause PTS error. Enter recovery process");
				...
				{
#if	!defined( __MAUI_BASIC__)
                    tst_sys_trace("RETRY_18V_ONLY_USIM");
#else
	...
	if(SimCard->simMagic1 != SimCard->simMagic2){ /*cmd finished before GPT timeout*/
		...
		if(0x60 == SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_SW1_MTK)){
...
#if	!defined( __MAUI_BASIC__)
            tst_sys_trace("[SIM] : null timeout\n\r");
#else
            dbg_print("[SIM] : null timeout\n\r");
#endif	
			...
			if(88 < SimCard->sim_nullByteIssuenonNullCount){
				...
				SIM_DisAllIntr();
#if	!defined( __MAUI_BASIC__)
				tst_sys_trace("[SIM] : non null timeout\n\r");
#else
...
#if	!defined( __MAUI_BASIC__)
         tst_sys_trace("[SIM] : non-null byte\n\r");
#else
         dbg_print("[SIM] : non-null byte\n\r");
#endif	
...
#if defined(SIM_DEBUG_INFO)
#if	!defined( __MAUI_BASIC__)
            tst_sys_trace("!!! ERR  NULL rx buffer \r\n");
#else
