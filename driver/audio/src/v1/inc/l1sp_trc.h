...
BEGIN_TRACE_MAP(MOD_L1SP)
   ...
   TRC_MSG(L1SP_ADAPT_MIC_GAIN_CONTROL,            "[MIC GAIN CONTROL]L1SP adapt mic gain control %x")   
	TRC_MSG(L1SP_MUTE_DSP_UL_CONTROL,            "[MUTE] L1SP_MuteMicrophone(inAM): %x")  
	TRC_MSG(L1SP_MUTE_DSP_DL_CONTROL,            "[MUTE] L1SP_MuteSpeaker(inAM): %x")  
...
// 3G speech
   ...
   TRC_MSG(ECALL_PSAP_MODEM_ROBUST_MODULATION, "[eCall][PSAP] robust modulator chosen (metric: %d)")
	...
	// HW side tone
   TRC_MSG(SIDETONE_START, "Enter SIDETONE_UpdateStart")
   TRC_MSG(SIDETONE_STOP, "Enter SIDETONE_UpdateStop")
