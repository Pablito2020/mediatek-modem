...
void SALI_Set_2G_Sch_Delay(uint8 codec_mod, uint8 subchannel)
{
   ...
   SALI_Set_Sch_Delay(delR, delW, delM, delay);
}
...
void SALI_Set_2G_Handover()
{
            ...
            *DSP_SPH_COD_CTRL |= 0x8;
			L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(6), (fn - sal_impl.ho_fn)>13, *DSP_SPH_COD_CTRL, (uint16)(fn>>16), (uint16)fn);
			sal_impl.ho_fn = fn;
}
...
void SALI_Set_Sch_Delay(uint16 delR, uint16 delW, uint16 delM, uint16 delO)
{
   ...
   *DSP_SPH_DEL_O = delO;
   	...
   	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(3), delR, delW, delM, delO);
}
...
void SALI_Config_SRC(uint8 apptype, bool isWB)
{
   ...
   case  SAL_DEV_NORMAL:
      if(isWB || sal_impl.fwe){
         ...
         #if _SAL_FO_DUALMIC_SUPPORT_ //wayn come back
         v |= BIT6;
         #endif          
      }   
...
//#if defined(MT6280) || defined(MT6589) || defined(MT6572) || defined(MT6582)
#define BGSND_PAGE_NUM 5
//#endif
...
volatile uint16* SALI_Bgsnd_GetBuf(uint8 ul_dl)
{
	volatile uint16* ptr;
	if(ul_dl == 0)
	ptr = DSP_DM_ADDR(BGSND_PAGE_NUM, *DSP_SPH_BGS_ADDR_UL);
	else
	ptr = DSP_DM_ADDR(BGSND_PAGE_NUM, *DSP_SPH_BGS_ADDR_DL);		
...
void SALI_Init()
{
	...
	*DSP_SPH_SCH_IMPROVE_CTRL |= 0x4405;
	...
	#if _SAL_FO_DUALMIC_SUPPORT_
    *DSP_SPH_DUMIC_CTRL |= 0x0001;
    #endif 
    
  sal_impl.resync_8k = true;//DSP allways supports, real control from L1's gps co-clock feature

	#if (defined(MT6280) || defined(MT6589) || defined(MT6572) || defined(MT6582) || defined(MT6592) || defined(MT6571))
	sal_impl.fwe = true;
...
void SALI_ENH_Dynamic_Ctrl(bool on, Sal_Enh_Dynamic_t fea)
{
	...
	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(5), on, fea, *DSP_SPH_ENH_DYMANIC_SWITCH, SAL_UNKNOWN);
}
...
void SALI_PcmEx_Config(Sal_PCMEx_Config_t* cfg)
{
	if(cfg->type == SAL_PCMEX_TYPE_PNW){
		...
		if(cfg->M2D_ul)
			*DSP_SPH_PNW_CTRL_UL |= (0x0001 << 11);
		else
			*DSP_SPH_PNW_CTRL_UL &= ~(0x0001 << 11);
		if(cfg->D2M_ul)
			*DSP_SPH_PNW_CTRL_UL |= (0x0001 << 10);
		else
			*DSP_SPH_PNW_CTRL_UL &= ~(0x0001 << 10);			
		if(cfg->afterEnh_ul)
			*DSP_SPH_PNW_CTRL_UL |= (0x0001 << 9);
		else
			*DSP_SPH_PNW_CTRL_UL &= ~(0x0001 << 9);		
		
		if(cfg->M2D_dl)
			*DSP_SPH_PNW_CTRL_DL |= (0x0001 << 11);
		else
			*DSP_SPH_PNW_CTRL_DL &= ~(0x0001 << 11);		
		if(cfg->D2M_dl)
			*DSP_SPH_PNW_CTRL_DL |= (0x0001 << 10);
		else
			*DSP_SPH_PNW_CTRL_DL &= ~(0x0001 << 10);
		if(cfg->afterEnh_dl)
			*DSP_SPH_PNW_CTRL_DL |= (0x0001 << 9);
		else
			*DSP_SPH_PNW_CTRL_DL &= ~(0x0001 << 9);
...
bool SALI_PcmEx_IsCtrlClean(Sal_PcmEx_Type_t type)
{
	...
	if(type == SAL_PCMEX_TYPE_PNW)
		ret = (*DSP_SPH_PNW_CTRL_UL == 0) && (*DSP_SPH_PNW_CTRL_DL == 0);
...
{
	#if defined(MT6280)
	eplfrm->ul_pre_buf = DSP_DM_ADDR(6, 0x03C0);
	eplfrm->ul_pos_buf = DSP_DM_ADDR(6, 0x0280);
	eplfrm->dl_pre_buf = DSP_DM_ADDR(6, 0x0000);
	eplfrm->dl_pos_buf = DSP_DM_ADDR(6, 0x0140);
...
void SALI_Mute_Ctrl(Sal_Mute_Point_t ctrl, bool mute)
{
	...
	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(2), ctrl, mute, SAL_UNKNOWN, SAL_UNKNOWN);
