...
{
	kal_trace(TRACE_INFO, L1SP_MUTE_DSP_UL_CONTROL, mute);
...
void AM_MuteDLSpeech( bool mute )
{
	kal_trace(TRACE_INFO, L1SP_MUTE_DSP_DL_CONTROL, mute);
	// SAL_Mute_Ctrl(SAL_MUTE_DL_PRE_EN, mute);
...
static void AM_DSP_BluetoothOff( uint32 data , uint32 extraInfo)
{
   ...
   AM_DSP_SetSpeechUplinkDigitalGain(am.bt_mic_gain); 
   ...
   switch(type)
   {
      ...
      case AM_PCMEX_TYPE_IDLE: //2: PCM2WAY VoIP
	      if(flag){
			...
			cfg.D2M_ul = (0!=(pcmExData->cfgUL & USE_D2M_PATH));
			cfg.M2D_ul = (0!=(pcmExData->cfgUL & USE_M2D_PATH));
			cfg.afterEnh_ul = (0!=(pcmExData->cfgUL & DATA_SELECT_AFTER_ENH));
			...
			if(cfg.idle){
				if(pcmExData->bandInfo == PCMNWAY_BAND_INFO_WB){
					cfg.band = SAL_PCM_WIDEBAND;			 
				}else if (pcmExData->bandInfo == PCMNWAY_BAND_INFO_NB){
