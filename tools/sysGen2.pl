...
# Constants
#****************************************************************************
my $SYSGEN2_VERNO     = " m0.21";
                       #  m0.21 , 2013/04/11 by mei, Support attribute got from custom_scatstruct.h
                       #  m0.20 , 2012/11/17 by mei, Support dump VECTOR_TABLE with ROM region and refactory some functions
...
sub RVCT_Process
{
    ...
    $g_DumpRegions_ref = $g_MAUILinkInfo->GetDumpExeRegions($g_bb);
...
{
    my $strSDSBase = "0x0";
    if(!FileInfo::is_SmartPhone() and FileInfo::is("sds_support", "TRUE"))
    {
        my $nFlashEnd = &memConfig::GetPhysicalFlashEnd();
...
sub SECURE_CFG_H_Gen_SDS_RESERVEDSIZE
{
    ...
    if(!(&FileInfo::is_NOR() or &FileInfo::is_SmartPhone()) and FileInfo::is("sds_support", "TRUE"))
    {
...
sub SCAT_H_Gen_DUMP_REGION_COUNT
{
    ...
    {
        $g_DumpRegions = $g_MAUILinkInfo->GetDumpExeRegions($g_bb);
    }
...
sub SCAT_C_Gen_TEMPLATE_DUMP_REGION_ASSIGN
{
    ...
    for(my $i = 0; $i < $#$DumpRegions_ref+1; $i++)
    {
        ...
        elsif($GroupName =~ /VECTOR_TABLE/i)
        {
...
sub Get_Dump_Regions
{
    ...
    {
        my $array_ref = $g_DumpRegions_ref->{$strGroupName};
