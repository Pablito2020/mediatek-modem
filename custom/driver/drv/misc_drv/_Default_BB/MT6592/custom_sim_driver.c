...
*/
  {0, SIM_ICC_APPLICATION_PHONE1, MTK_SIMIF0, SIM_SWITCH_NONE, 0, 0},
  {1, SIM_ICC_APPLICATION_PHONE2, MTK_SIMIF1, SIM_SWITCH_NONE, 0, 1},
};
   ...
   if (iccHotPlugTable[idx].polarity == LEVEL_HIGH)
   {
      ...
      if (iccHotPlugTable[idx].plugOutcb != NULL)
      {
	   ...
	   iccHotPlugTable[idx].polarity = LEVEL_LOW;
...
#endif
		kal_sprintf(sim_dbg_custom_str,"[SIM_CUS_DRV]Remove SIM : %x, %x, %x, %x, %x, %x, %x",
		idx,
		app, 
		...
		sim_dbg_print(sim_dbg_custom_str);
#endif
   }
   else
   {
...
#endif
		kal_sprintf(sim_dbg_custom_str,"[SIM_CUS_DRV]Insert SIM : %x, %x, %x, %x, %x, %x, %x",
		idx,
		app, 
		iccHotPlugTable[idx].polarity, 
		iccHotPlugTable[idx].removed, 
                           ...
                           kal_uint32 socketType)
{
	...
	for(loopIndex = 0; loopIndex < iccSlotNum; loopIndex++){
		if(application == iccMappingTable[loopIndex].application){
		   ...
		   {
				sim_dbg_print("[SIM_DRV]sim_registed");
		      return;
		   }
		   else
		   {
   ...
   /* need to set SW debounce & sensitivity before registration */
	EINT_SW_Debounce_Modify(eintNo, debounceTime);// 32768Hz, 
